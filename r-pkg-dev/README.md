...
- **`r-conventions` skill.** The full set of R package conventions (R code, dependencies, `DESCRIPTION`, data, testing, documentation, lifecycle, license) plus agent-workflow guidance for building packages efficiently (`load_all()` over reinstall, the tight test loop). It triggers automatically when working in an R package (editing `.R`/`.Rmd`/`.qmd`, `DESCRIPTION`, `NAMESPACE`, `NEWS.md`, or `_pkgdown.yml`, or running R tests).
