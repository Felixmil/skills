...
- Before committing, get the whole suite green: a guardrail hook shipped with this plugin runs the full `devtools::test()` on `git commit` and blocks the commit if any test fails, so run the full suite yourself first (at a natural stopping point, not after every edit) rather than being surprised by the gate.
