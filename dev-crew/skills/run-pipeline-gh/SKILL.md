...
6. **Artifact-approval gate** per `${CLAUDE_PLUGIN_ROOT}/references/pipeline/modes.md`: in `manual` mode, first summarise the deliverable per that reference's "Summarising the deliverable at a manual gate" (the artifact you read back in step 5 is the source; because you have already delivered to GitHub in step 5, the link is the **GitHub URL of the delivered copy**, the tagged issue-comment URL for spec/plan, the pull-request URL for build/QA, per the GitHub-delivery link form), then stop for an approve/revise decision before advancing; in `auto`/`semi-auto`, advance immediately after delivering.
