...
6. **Artifact-approval gate** per `${CLAUDE_PLUGIN_ROOT}/references/pipeline/modes.md`: in `manual` mode, first summarise the deliverable per that reference's "Summarising the deliverable at a manual gate" (the artifact you just read back in step 5 is the source; the link is the **absolute filesystem path** of that artifact, `<root>/<issue>/<phase>.md`, per the file-delivery link form), then stop for an approve/revise decision before advancing; in `auto`/`semi-auto`, advance immediately.
