...
- **Guardrail hooks**, all no-ops outside an R package so they are safe everywhere:
  ...
  The test-suite and R CMD check gates also cache their last pass: they record the fingerprint of the R-relevant content that passed (per repository, in `~/.claude/r-dev/gate-cache.json`) and skip a re-run when a retried commit or push ships byte-identical R content. This is what stops a repeated commit/push (after fixing an unrelated failure such as a rejected `gh pr create` or a message typo) from re-running a minutes-long check that already passed. Any change to an R file changes the fingerprint, so the gate runs again; a missing or corrupt cache simply means the gate runs. The commit cache covers a plain `git commit`; `-a` and `--amend` are never cached. While a gate runs, the progress line names it (e.g. "running R CMD check before push"), so a slow commit/push reads as the gate working rather than a hang.

  The commit and push gates have a deliberate escape hatch: prefix the command with `R_PKG_GATE_SKIP=1` (e.g. `R_PKG_GATE_SKIP=1 git commit -m wip`, `R_PKG_GATE_SKIP=1 git push`) to bypass them for that one command. The bypass is announced on stderr, so it is always a conscious, visible choice, not a silent skip.
