---
name: r-pkg-dev
description: Conventions for ALL R work, in an R package or a plain R project. A repo with a DESCRIPTION file is an R package; if the current repo is one, load this as soon as the session touches it, without waiting for a specific file to be named. MUST be loaded whenever the work involves R at all: writing, editing, running, testing, reviewing, or critiquing R code, or working in an R package repo. Covers R code style, functions, dependencies, DESCRIPTION/NAMESPACE, data, testing (testthat), documentation (roxygen2, NEWS.md, pkgdown, vignettes), lifecycle, and license. Package-only rules (DESCRIPTION/NAMESPACE/NEWS/roxygen/pkgdown/lifecycle) are marked inside and are inert in a non-package project; the code-style and testing rules apply to all R code. When in doubt whether the work is R, load it.
when_to_use: Editing, creating, or running any .R, .r, .Rmd, or .qmd file, or DESCRIPTION, NAMESPACE, NEWS.md, _pkgdown.yml; running R code, R tests, R CMD check, devtools, usethis, testthat, roxygen2, renv, or pak; writing or changing an R function, S3/S4/R6 method, or test. Reviewing or critiquing R code even when "R" is not named, a pull request, a diff, or a snippet whose files are R. Working inside a repo that is an R package (has a DESCRIPTION), including at session start, a review, or a PR on that repo. Anything mentioning CRAN, tidyverse, an R package, or an R build.
---
